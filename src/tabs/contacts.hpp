		...
		namespace contacts {
			...
			bool AddContactDialog(simple_db::TableSession* db) {
				...
				if (ImGui::CollapsingHeader("Phone Numbers")) {
					...
					Tooltip("Add new phone number field");
					ImGui::SameLine();
					...
					Tooltip("Remove last added number");
				}
				if (ImGui::CollapsingHeader("Emails")) {
					...
					Tooltip("Remove last added email");
				}
				if (ImGui::Button("Cancel##adding contact")) {
					busy = false;
				}
				Tooltip("Stop creating contact.\nAll data will be lost!");
				ImGui::SameLine();
			...
			void ContactHeader(Contact contact) {
				...
				Tooltip("Go back to the contact list");
